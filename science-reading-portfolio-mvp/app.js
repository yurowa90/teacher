/* ============================================================
   과학 독서·논술 포트폴리오 — Stage 1 MVP (Vanilla JS)
...
function studentProgress(studentId) {
  ...
  return { done, total: activities.length, ratio: done / activities.length };
}

function activitySubmissionRate(activityId) {
  const done = students.filter(s => isSubmitted(s.studentId, activityId)).length;
  return { done, total: students.length, ratio: done / students.length };
}
...
/* ------------------------------------------------------------
   6) 리뷰 패널 (답안 검토 · 루브릭 · 오개념 · 피드백)
   ------------------------------------------------------------ */

let reviewDraft = null; // { studentId, activityId, scores, misconceptionIds }
...
function openReviewPanel(studentId, activityId) {
  ...
  const answer = art && art.content && art.content.trim().length > 0
    ? esc(art.content)
    : '';
  ...
  // 선택된 오개념에 대응하는 참고 피드백 문구 (교사 판단 보조용, 자동 확정 아님)
  const suggested = misconceptions
  ...
  el.reviewPanel.innerHTML = `
    ...
    <div class="suggested-feedback" data-suggested>
      선택한 오개념 참고 문구 (교사가 판단·수정하는 보조 자료입니다):
      <ul>${suggested || '<li>선택된 오개념이 없습니다.</li>'}</ul>
    </div>
  ...
  el.reviewPanel.querySelectorAll('[data-misc-input]').forEach(input => {
    input.addEventListener('change', () => {
      ...
      // 참고 문구 갱신
      const box = el.reviewPanel.querySelector('[data-suggested]');
      const items = misconceptions
        .filter(mc => reviewDraft.misconceptionIds.includes(mc.id))
        .map(mc => `<li>${esc(mc.feedback)}</li>`).join('');
      box.innerHTML = `선택한 오개념 참고 문구 (교사가 판단·수정하는 보조 자료입니다):
  ...
  el.reviewPanel.querySelector('#btn-save-review').addEventListener('click', () => {
    ...
    status.textContent = '저장되었습니다 · ' + new Date().toLocaleTimeString('ko-KR');
    // 대시보드 지표 갱신
    renderMetricCards();
    renderMisconceptionFreq();
  });
