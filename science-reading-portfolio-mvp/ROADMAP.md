...
- **교사가 임의의 책·성취기준으로 단원 저작**: 활동·루브릭·오개념 CRUD
  (생물학/진화 세트는 '샘플 불러오기' 예시 템플릿일 뿐 고정 내용 아님)
- 응답·리뷰·루브릭·오개념·피드백을 Supabase DB에 저장
- 교사 단원 페이지에서 진행률·제출 여부 확인, 답안 검토·채점
- **RLS 정책**: 학생은 자기 응답만, 교사는 자기 반·자기 단원만
  (로컬 PostgreSQL 16에서 격리·위조 차단·교사 저작 격리 검증 완료)
- `join_class` security definer RPC로만 초대 코드 합류(임의 자가등록 차단)
- **유지 제한**: 실명·학번·민감정보 미수집 / AI 자동채점·세특 자동 확정 없음 / 교사 최종 판단

**남은 Stage 3+ 확장**: 오개념 빈도·평균 점수 차트, CSV 서버 내보내기,
포트폴리오 증거(E) 묶음 뷰, 활동 순서 드래그 정렬.
