...
2. 앱 상단 **0 · 실행 방식 → Gemini API 자동 호출**을 고르고 키를 붙여넣습니다.
   ...
   - 모델 기본값은 `gemini-flash-latest`(항상 최신 flash를 가리키는 별칭)입니다.
   - **404·"모델 없음" 오류**가 나면 모델 칸 옆 **모델 불러오기** 버튼을 눌러
     이 키로 실제 사용 가능한 모델을 목록에서 고르세요. (예전 `gemini-2.5-flash`는
     신규 사용자에게 더 이상 제공되지 않아 404가 납니다.)
3. **문항 · 루브릭 생성**을 누릅니다.
