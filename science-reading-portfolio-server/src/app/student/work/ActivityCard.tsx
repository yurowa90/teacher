...
export default function ActivityCard({ studentId, activity, initialContent, dataTable }: Props) {
  ...
  return (
    <article className="card">
      <div className="row" style={{ gap: 8, marginBottom: 6 }}>
        ...
        <span className="muted" style={{ fontSize: 13 }}>{activity.level}</span>
      </div>
      <strong>{activity.title}</strong>
      <p className="muted" style={{ marginTop: 4 }}>{activity.prompt}</p>

      {dataTable && (
        <div style={{ overflowX: 'auto', margin: '10px 0' }}>
          <table>
            <thead>
              <tr>{dataTable.headers.map((h) => <th key={h}>{h}</th>)}</tr>
            </thead>
            <tbody>
